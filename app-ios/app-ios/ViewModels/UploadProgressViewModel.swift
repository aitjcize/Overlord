    ...
    func setupWebSocketHandlers() {
        webSocketService.on(event: "upload started") { [weak self] message in
            guard let self = self,
                  ...
                  let uploadInfo = try? JSONDecoder().decode(UploadStartInfo.self, from: data)
            else {
                return
            }
        ...
        webSocketService.on(event: "upload progress") { [weak self] message in
            guard let self = self,
                  let data = message.data(using: .utf8),
                  let progressInfo = try? JSONDecoder().decode(UploadProgressInfo.self, from: data)
            else {
                return
            }
        ...
        webSocketService.on(event: "upload completed") { [weak self] message in
            guard let self = self,
                  let data = message.data(using: .utf8),
                  let completionInfo = try? JSONDecoder().decode(UploadCompletionInfo.self, from: data)
            else {
                return
            }
        ...
        webSocketService.on(event: "upload failed") { [weak self] message in
            guard let self = self,
                  let data = message.data(using: .utf8),
                  let failureInfo = try? JSONDecoder().decode(UploadFailureInfo.self, from: data)
            else {
                return
            }
